...
Optional finding-create flags include `--city`, `--region`, `--term`, `--terms-json`, `--start-date`, `--end-date`, `--location-raw`, `--source-url`, `--discovered-at`, `--posted-age`, `--fit-notes`, `--policy-blocker`, `--disposition-reason`, and `--merge-status`.
...
Bulk import rows may also include `term` or `terms` for term-based timing. Do not mix `term`/`terms` with `startDate`/`endDate` in the same row.
