...
3. Inspect before mutating:
   ...
   - Treat `create`, `update`, `status`, `archive`, `note`, `workflow`, `link add/update`, `attempts`, `scores record`, `runs start/step/complete`, `sourcing run`, `sourcing run --auto-promote`, `sourcing findings create/decide/import/update/promote`, `profile update`, `profile sensitive update`, and `profile secrets upsert/delete` as mutations.
