...
describe('raw sourcing intake', () => {
  ...
  it('keeps a Jobright intermediary sparse and pending for server-side resolution', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body))).toEqual({ records: [{ adapter: { id: 'valedictorian-cli', kind: 'cli', version: '0.1.0-alpha.14' }, observedAt: '2026-07-12T12:00:00.000Z', payload: { url: 'https://jobright.ai/jobs/2' } }] })
