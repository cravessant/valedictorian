...
describe('raw sourcing intake', () => {
  ...
  it('ingests a sparse URL observation with CLI adapter provenance', async () => {
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body))).toEqual({ records: [{
      intakeItemId: 'cli-intake-1',
      adapter: { id: 'valedictorian-cli', kind: 'cli', version: '0.1.0-alpha.17' },
  ...
  it('keeps a Jobright intermediary sparse and pending for server-side resolution', async () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(JSON.parse(String(fetchMock.mock.calls[0]?.[1]?.body))).toEqual({ records: [{ intakeItemId: 'cli-intake-1', adapter: { id: 'valedictorian-cli', kind: 'cli', version: '0.1.0-alpha.17' }, observedAt: '2026-07-12T12:00:00.000Z', payload: { url: 'https://jobright.ai/jobs/2' } }] })
  ...
  it('renders provenance, intake, actual statuses, and safe inspection failure for humans', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stdout).toContain('Provenance (submitted): adapter=valedictorian-cli kind=cli version=0.1.0-alpha.17 reportedOrigin=job_board:Jobright')
