...
export function parseActionQueueListQuery(argv: string[]): ActionQueueListQuery {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--action-bucket') {
      ...
      if (!isActionQueueBucket(bucket)) throw new CliUsageError(`Invalid action queue bucket: ${bucket}`)
    ...
    if (token === '--limit') {
      query.limit = parseStrictNonNegativeIntegerOption(
        readRequiredArgument(argv[index + 1], '--limit value'),
        '--limit',
      )
    ...
    if (token === '--offset') {
      query.offset = parseStrictNonNegativeIntegerOption(
        readRequiredArgument(argv[index + 1], '--offset value'),
        '--offset',
      )
      index += 1
      continue
...
export function parseWorkflowRunsListQuery(
  argv: string[],
): NonNullable<Parameters<ValedictorianWorkspaceClient['runs']['list']>[0]> {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--run-type') {
      ...
      if (!isRunType(runType)) throw new CliUsageError(`Invalid run type: ${runType}`)
    ...
    if (token === '--status') {
      ...
      if (!isRunStatus(status)) throw new CliUsageError(`Invalid run status: ${status}`)
      query.status = status
      index += 1
      continue
    }
    if (token === '--source') query.source = readRequiredArgument(argv[++index], '--source value')
...
export function parseRunStart(
  ...
  if (!isRunType(runType)) throw new CliUsageError(`Invalid run type: ${runType}`)
  if (!workflowActorTypes.has(actorType)) throw new CliUsageError(`Invalid actorType: ${actorType}`)

  const input: Parameters<ValedictorianWorkspaceClient['runs']['start']>[0] = {
    runType,
    actorType: actorType as 'agent',
  }
  ...
  if (started !== undefined) input.coverageStartedAt = parseNullableTimestampOption(started, 'coverageStartedAt')
  if (ended !== undefined) input.coverageEndedAt = parseNullableTimestampOption(ended, 'coverageEndedAt')
  if (inputJson !== undefined) input.input = parseStrictJsonValue(inputJson, '--input-json')
  if (metadataJson !== undefined) input.metadata = parseStrictJsonValue(metadataJson, '--metadata-json')
  return input
}
...
export function parseRunStep(
  ...
  if (payloadJson !== undefined) input.payload = parseStrictJsonValue(payloadJson, '--payload-json')
  return input
}
