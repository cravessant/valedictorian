...
export function parseSourcingFindingsListQuery(
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--workflow-run-id') query.workflowRunId = readRequiredArgument(argv[++index], '--workflow-run-id value')
      ...
      if (!isSourcingMergeStatus(status)) throw new CliUsageError(`Invalid merge status: ${status}`)
      query.mergeStatus = status
    } else if (token === '--limit') {
      query.limit = parseStrictNonNegativeIntegerOption(
        readRequiredArgument(argv[++index], '--limit value'),
        '--limit',
      )
      validateLimit(query.limit)
    } else if (token === '--offset') {
      query.offset = parseStrictNonNegativeIntegerOption(
        readRequiredArgument(argv[++index], '--offset value'),
        '--offset',
      )
    } else if (token !== '--json') throw new CliUsageError(`Unknown option: ${token}`)
  }
  return query
}
...
export function parseSourcingFindingDecision(
  ...
  if (!isManualSourcingDecisionStatus(mergeStatus)) throw new CliUsageError(`Invalid manual sourcing decision: ${mergeStatus}`)
...
function setOptionalTimingOptions(
  ...
  if (input.term !== undefined || input.terms !== undefined || input.startDate !== undefined || input.endDate !== undefined) asCliUsage(() => normalizeJobTimingInput(input))
}
...
function assertWritableSourcingMergeStatus(mergeStatus: string): asserts mergeStatus is Exclude<ReturnType<typeof writableStatus>, undefined> {
  if (!isSourcingMergeStatus(mergeStatus)) throw new CliUsageError(`Invalid merge status: ${mergeStatus}`)
  if (mergeStatus === 'merged') throw new CliUsageError('Sourcing findings can only be marked merged by promotion.')
}
...
function assertCompatibleDisposition(input: { blocker?: string | null; dispositionReason?: string | null; duplicateNotes?: string | null; mergeStatus?: string; policyBlocker?: string | null }) {
  if (input.duplicateNotes !== undefined) throw new CliUsageError('--duplicate-notes is generated by duplicate detection; use --disposition-reason or --merge-notes for manual notes.')
  if (input.blocker !== undefined && input.mergeStatus !== 'blocked') throw new CliUsageError('--blocker requires --merge-status blocked.')
  if (input.policyBlocker !== undefined && input.mergeStatus !== 'blocked') throw new CliUsageError('--policy-blocker requires --merge-status blocked.')
  if (input.dispositionReason !== undefined && (input.mergeStatus === undefined || !isManualSourcingDecisionStatus(input.mergeStatus))) throw new CliUsageError('--disposition-reason requires a manual --merge-status.')
}
