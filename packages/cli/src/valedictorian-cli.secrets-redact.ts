const DEFAULT_REDACTION_MARKERS = ['[redacted]', '⟦redacted⟧', '<<REDACTED>>'] as const

export function redactExactValues(text: string, values: readonly string[]): string {
  const uniqueNonEmpty = [...new Set(values.filter((value) => value.length > 0))].sort(
...
function chooseRedactionMarker(values: readonly string[]): string {
