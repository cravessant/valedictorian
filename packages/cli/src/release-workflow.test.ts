...
describe('CLI release workflow', () => {
  ...
  it('documents the alpha install tag and explicit workspace commands', () => {
    ...
    expect(readme).toContain('pnpm add -g valedictorian-cli@alpha')
    ...
    expect(skill).toContain('pnpm add -g valedictorian-cli@alpha')
    expect(skill).toContain('Workspace-scoped commands require `--workspace <id-or-name>`')
    expect(commandReference).toContain('pnpm add -g valedictorian-cli@alpha')
