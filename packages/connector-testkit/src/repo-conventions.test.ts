...
describe("connector repository conventions", () => {
  it("documents package boundaries and the API-only Jobright architecture", () => {
    ...
    expect(readme).toContain("fixed nominal `ConnectorExecutionError`")
    expect(readme).toContain("without treating the failure as an ordinary refresh result")
    expect(readme).toContain("`sanitizeConnectorRefreshWarnings(...)`")
    expect(readme).toContain("`sanitizeConnectorRefreshStats(...)`")
    expect(readme).toContain("`sanitizeConnectorSynchronization(...)`")
    expect(readme).toContain("This issue is not fully integrated in the app")
    ...
    expect(readme).toContain("`connector.progress_reporting_failed`")
  ...
  it("documents connector package publishing and versioning expectations", () => {
    ...
    expect(readme).toContain("The current breaking release tree is `0.17.0`")
    expect(readme).toContain("exact `workspace:^0.17.0` internal compatibility ranges")
  ...
  it("keeps current packages public and separated by dependency direction", () => {
    ...
    expect(harnessPackage.dependencies).toEqual({
      "@sparxie/valedictorian-connectors-core": "workspace:^0.17.0",
      sparxie: "0.27.0",
    })
    ...
    expect(jobrightPackage.devDependencies).toMatchObject({
      "@sparxie/valedictorian-connectors-test-harness": "workspace:^0.17.0",
    })
  })
