...
/** Watcher primitive for externally edited local profile files. */
...
export function createProfileJsonWatcher(
  ...
  const setTimeoutFn: NonNullable<ProfileJsonWatchOptions['setTimeoutFn']> =
    options.setTimeoutFn ?? ((callback, ms) => setTimeout(callback, ms))
  const clearTimeoutFn: NonNullable<ProfileJsonWatchOptions['clearTimeoutFn']> =
