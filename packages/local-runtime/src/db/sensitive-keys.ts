 ...
 *
 * The schema CHECKs reject any JSON whose object keys CONTAIN a sensitive
 * substring, not just exact matches — so OAuth- and header-style keys
 * (`access_token`, `refresh_token`, `client_secret`, `api_key`/`apiKey`,
 * `X-Api-Key`, `X-Auth-Token`, `privateKey`, `bearer`, `credential`, …) are
 * caught. Matching is case-insensitive and underscore/hyphen/camelCase agnostic.
 *
 * This is deliberately STRICTER than the sparxie contract's exact-key rule
 * (defense in depth). Runtime writers own their key choices.
 */
