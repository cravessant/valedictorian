...
export function seedDoomedConnectorRunFixture(database: TestDatabase) {
  database.exec(`
    insert into connector_instances (id, connector_id, connector_version, display_name, enabled, config_json, auth_json, filters_json, created_at, updated_at)
    values ('doomed-instance', 'fixture.doomed', '1.0.0', 'Doomed', 1, '{}', '[]', '{}', '2026-07-10T12:00:00.000Z', '2026-07-10T12:00:00.000Z');
    insert into connector_runs (id, connector_instance_id, mode, status, started_at, completed_at, config_json, filters_json, filter_signature, observation_count, warning_count, stats_json, warnings_json, retry_hints_json, created_at, updated_at)
    values ('doomed-run', 'doomed-instance', 'manual', 'partial_success', '2026-07-10T12:00:00.000Z', '2026-07-10T12:00:01.000Z', '{}', '{}', 'filters:{}', 1, 0, '{}', '[]', 'null', '2026-07-10T12:00:00.000Z', '2026-07-10T12:00:01.000Z');
    ...
    insert into raw_source_occurrences (id, raw_record_id, raw_revision_id, connector_instance_id, connector_run_id, observed_at, received_at)
    values ('preserved-occurrence', 'doomed-record', 'doomed-revision', 'doomed-instance', 'doomed-run', '2026-07-10T12:00:00.000Z', '2026-07-10T12:00:01.000Z');
    ...
    insert into retry_work (id, kind, connector_instance_id, filter_signature, checkpoint_schema_version, checkpoint_generation, reason, attempt, max_attempts, last_attempt_at, computed_delay_ms, next_attempt_at, horizon_at, state, owner_version, lineage_json, acquired_at, acquisition_token, acquisition_run_id, skipped_run_id, created_at, updated_at)
    values ('doomed-retry', 'connector_capture', 'doomed-instance', 'filters:{}', 'fixture@1', '1.0.0', 'server_failure', 1, 3, '2026-07-10T12:00:00.000Z', 1000, '2026-07-10T12:00:01.000Z', '2026-07-10T13:00:00.000Z', 'acquired', '1.0.0', '{}', '2026-07-10T12:00:00.000Z', 'token', 'doomed-run', 'doomed-run', '2026-07-10T12:00:00.000Z', '2026-07-10T12:00:00.000Z');
  `)
}
