...
describe('source execution scope ownership invariants', () => {
  it('rejects connector runs, occurrences, and retry ownership bound to a wrong existing scope', async () => {
    ...
    sqlite.exec(`insert into capture_lineages (id,created_at) values ('record','2026-07-12T00:00:00.000Z'); insert into capture_evidence_versions (id,capture_lineage_id,revision,content_hash,adapter_id,adapter_kind,adapter_version,observed_at,evidence_json,created_at) values ('revision','record',1,'hash','fixture','connector','1','2026-07-12T00:00:00.000Z','[]','2026-07-12T00:00:00.000Z')`)
    expect(() => sqlite.exec(`insert into captures (id,capture_lineage_id,capture_evidence_version_id,connector_instance_id,connector_run_id,execution_scope_id,observed_at,received_at) values ('occurrence','record','revision','one','run-one','${two.executionScopeId}','2026-07-12T00:00:00.000Z','2026-07-12T00:00:00.000Z')`))
      .toThrow(/scope owner mismatch/)
    sqlite.exec(`insert into captures (id,capture_lineage_id,capture_evidence_version_id,connector_instance_id,connector_run_id,execution_scope_id,observed_at,received_at) values ('occurrence','record','revision','one','run-one','${one.executionScopeId}','2026-07-12T00:00:00.000Z','2026-07-12T00:00:00.000Z')`)
    expect(() => sqlite.exec(`update captures set execution_scope_id='${two.executionScopeId}' where id='occurrence'`)).toThrow(/scope owner mismatch/)
