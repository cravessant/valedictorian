...
async function seedJobrightCapture(database: Database, jobResult: Record<string, unknown>, recordId: string) {
  ...
  const accepted = await captures.accept({
    workspaceId: WS,
    provenance: { adapterId: ADAPTER_ID, adapterKind: 'connector', adapterVersion: '0.18.0', providerRecordId: recordId, providerSchema: PROVIDER_SCHEMA, observedAt: '2026-07-22T10:00:00.000Z' },
...
function promotion(database: Database, fieldOutcomes: ReturnType<typeof createCaptureFieldOutcomeStore>) {
  ...
  const jobService = createPgliteJobService(database, { now: clock })
  return {
    jobService,
