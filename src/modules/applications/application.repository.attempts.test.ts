...
import { useResettablePgliteTestDatabase, createPgliteTestDatabase } from '../../test/pglite-test-owner'
...
describe.sequential('PGlite application repository workflow attempts', () => {
  ...
  it('rejects submitted completion without a passed final-review verification receipt', async () => {
    ...
    const repository = createPgliteApplicationRepository(database)
    const missingReceipt = await repository.startApplicationAttempt({
      applicationId: 'application-versant-platform',
      actorType: 'agent',
      actorName: 'codex',
      summary: 'Started application.',
