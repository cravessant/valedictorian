...
describe('connector registry', () => {
  ...
  it('matches published sparxie and connector package versions for Jobright API auth', () => {
    ...
    expect(appPackage.dependencies.sparxie).toBe('0.27.1')
    expect(appPackage.dependencies['@sparxie/valedictorian-connectors-jobright']).toBe('0.17.1')
    expect(appPackage.devDependencies['@sparxie/valedictorian-connectors-core']).toBe('0.17.1')
    expect(appPackage.devDependencies['@sparxie/valedictorian-connectors-test-harness']).toBe('0.17.1')
    ...
    expect(sparxiePackage.version).toBe('0.27.1')
    expect(jobrightPackage.version).toBe('0.17.1')
    expect(jobrightPackage.dependencies?.['@sparxie/valedictorian-connectors-core']).toBe('^0.17.1')
    expect(corePackage.version).toBe('0.17.1')
    expect(harnessPackage.version).toBe('0.17.1')
  })
  ...
  it('triggers the default production Jobright registry through the public local client', async () => {
    ...
    await client.connectors.create({
      id: 'jobright-default', connectorId: 'jobright.resolver', connectorVersion: '0.17.1',
