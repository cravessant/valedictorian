...
describe('connector workflow dependencies', () => {
  it('adopts the released progress and destination-projection contracts exactly', () => {
    ...
    expect(packageJson.dependencies['@sparxie/valedictorian-connectors-jobright']).toBe('0.14.0')
    expect(packageJson.devDependencies['@sparxie/valedictorian-connectors-core']).toBe('0.14.0')
    expect(packageJson.devDependencies['@sparxie/valedictorian-connectors-test-harness']).toBe('0.14.0')
  ...
  it('resolves connector packages from the public npm registry without alternate sources', () => {
    ...
    for (const name of CONNECTOR_PACKAGES) {
      ...
      expect(lockfile).toContain(`'${name}@0.14.0':`)
      expect(lockfile).toMatch(
        new RegExp(
    ...
    expect(lockfile).toMatch(
      /'@sparxie\/valedictorian-connectors-jobright@0\.14\.0':\n\s+dependencies:\n\s+'@sparxie\/valedictorian-connectors-core': 0\.14\.0/,
    )
    expect(lockfile).toMatch(
      /'@sparxie\/valedictorian-connectors-test-harness@0\.14\.0':\n\s+dependencies:\n\s+'@sparxie\/valedictorian-connectors-core': 0\.14\.0/,
    )
  })
