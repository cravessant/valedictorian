...
describe('connector workflow dependencies', () => {
  it('adopts the released progress and destination-projection contracts exactly', () => {
    ...
    expect(packageJson.dependencies['@sparxie/valedictorian-connectors-jobright']).toBe('0.18.1')
    expect(packageJson.devDependencies['@sparxie/valedictorian-connectors-core']).toBe('0.18.1')
    expect(packageJson.devDependencies['@sparxie/valedictorian-connectors-test-harness']).toBe('0.18.1')
    expect(packageJson.dependencies['@sparxie/sdk']).toBe('0.29.1')
  })
  ...
  it('resolves connector packages from the public npm registry without alternate sources', () => {
    ...
    for (const name of CONNECTOR_PACKAGES) {
      ...
      expect(lockfile).toContain(`'${name}@0.18.1':`)
      expect(lockfile).toMatch(
        new RegExp(
    ...
    expect(lockfile).toMatch(
      /'@sparxie\/valedictorian-connectors-jobright@0\.18\.1':\n\s+dependencies:\n\s+'@sparxie\/valedictorian-connectors-core': 0\.18\.1/,
    )
    expect(lockfile).toMatch(
      /'@sparxie\/valedictorian-connectors-test-harness@0\.18\.1':\n\s+dependencies:\n\s+'@sparxie\/sdk': 0\.29\.0\n\s+'@sparxie\/valedictorian-connectors-core': 0\.18\.1/,
    )
  })
