...
/**
 * Regenerates the single database baseline from src/db/schema.ts.
 *
 ...
 * decides the entry index from drizzle/meta, so the previous artifacts are removed
 ...
 * safeguards in src/db/baseline-triggers.sql are appended verbatim to the
...
export const TRIGGER_SOURCE_PATH = 'src/db/baseline-triggers.sql'
...
function run(repoRoot: string) {
  ...
  process.stdout.write(`Appended ${TRIGGER_SOURCE_PATH} to drizzle/${BASELINE_TAG}.sql\n`)
}
