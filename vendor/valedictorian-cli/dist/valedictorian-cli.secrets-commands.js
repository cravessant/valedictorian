...
import { makeCommand, optionFlags, optionValue, parseProfileSecretKind, requiredOption, workspaceClient, writeJson, } from './valedictorian-cli.command-runtime.js';
            ...
            upsert: makeCommand({
                ...
                run: async (context, flags, key) => {
                    ...
                    const input = {
                        ...
                        value: fs.readFileSync(readRequiredText(optionValue(flags, 'value-file'), '--value-file path'), 'utf8'),
                    };
...
function makeSecretsRunCommand() {
    return makeCommand({
        docs: {
            ...
            fullDescription: 'Requires at least one explicit injection destination. Pass the executable and argv after --.',
        },
        flags: {
...
function variadicOptionFlags(names) {
    const result = {};
