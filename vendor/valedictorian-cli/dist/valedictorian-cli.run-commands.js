...
import { parseRunComplete, parseRunStart, parseRunStep, parseWorkflowRunsListQuery, } from './valedictorian-cli.parsers.js';
        ...
        routes: {
            complete: makeCommand({
                ...
                run: async (context, flags, workflowRunId) => {
                    ...
                    writeJson(context, await client.runs.complete(parseRunComplete(workflowRunId, toArgvWithoutWorkspace(flags))));
                },
            }),
            list: makeCommand({
                ...
                run: async (context, flags) => {
                    const client = await workspaceClient(context, flags);
                    writeJson(context, await client.runs.list(parseWorkflowRunsListQuery(toArgvWithoutWorkspace(flags))));
                },
            }),
            start: makeCommand({
            ...
            step: makeCommand({
                ...
                run: async (context, flags, workflowRunId) => {
                    const client = await workspaceClient(context, flags);
                    writeJson(context, await client.runs.step(parseRunStep(workflowRunId, toArgvWithoutWorkspace(flags))));
                },
            }),
        },
    });
