...
import { ProfileDocumentHttpError, ValedictorianHttpError, profileDocumentErrorBodySchema, profileDocumentErrorStatusByCode, } from '@sparxie/sdk';
import { CliUsageError } from './valedictorian-cli.failures.js';
import { booleanFlags, makeCommand, optionFlags, optionValue, readJsonObjectFile, workspaceClient, writeJson, } from './valedictorian-cli.command-runtime.js';
    ...
    return buildRouteMap({
        ...
        routes: {
            ...
            format: makeCommand({
                ...
                run: async (context, flags) => {
                    const client = await workspaceClient(context, flags);
                    const expectedRevision = readRequiredText(optionValue(flags, 'expected-revision'), '--expected-revision');
                    await withProfileDocumentErrors(context, async () => {
                        writeProfileDocument(context, await client.profile.document.format({ expectedRevision }));
                    });
                },
            }),
            get: makeCommand({
            ...
            restore: makeCommand({
                docs: {
                    ...
                    fullDescription: 'Requires --confirm. Use --expected-revision null when restoring without a current revision.',
                },
                flags: {
            ...
            update: makeCommand({
                docs: {
                    ...
                    fullDescription: 'Requires --expected-revision. Writes the unified public profile patch and returns the new document.',
                },
                flags: optionFlags(['workspace'], ['input-json', 'expected-revision']),
                run: async (context, flags) => {
                    const client = await workspaceClient(context, flags);
                    const expectedRevision = readRequiredText(optionValue(flags, 'expected-revision'), '--expected-revision');
                    const profile = readJsonObjectFile(readRequiredText(optionValue(flags, 'input-json'), '--input-json path'), 'profile update input');
                    await withProfileDocumentErrors(context, async () => {
                        writeProfileDocument(context, await client.profile.document.update({ expectedRevision, profile }));
                    });
                },
            }),
...
function parseRestoreExpectedRevision(flags) {
...
async function withProfileDocumentErrors(_context, operation) {
    try {
        await operation();
    }
    catch (error) {
...
function asProfileDocumentCliError(error) {
...
function writeProfileDocument(context, document) {
...
function writeProfileValidateResult(context, result) {
    if (context.outputJson) {
        writeJson(context, result);
        return;
    }
    context.process.stdout.write(`Profile document is valid (schemaVersion=${result.schemaVersion}, revision=${result.revision})\n`);
}
function writeProfileRestoreResult(context, document) {
    const result = {
        restored: true,
    ...
    context.process.stdout.write(`Restored profile backup document (schemaVersion=${result.schemaVersion}, revision=${result.revision})\n`);
}
export function formatProfileDocumentHuman(document) {
    const lines = [
...
function populatedPublicProfileFacts(profile) {
    const entries = [];
