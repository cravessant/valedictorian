import { formatCompanyHumanOutput, } from './valedictorian-cli.company-output.js';
export function formatHumanOutput(value, options = {}) {
...
function isConnectorRun(record) {
    return (typeof record.connectorInstanceId === 'string' &&
        isPlainRecord(record.newestFrontier) &&
        isPlainRecord(record.historicalBackfill) &&
        isPlainRecord(record.outcome));
}
function formatConnectorRun(record) {
    const outcome = record.outcome;
    const newest = record.newestFrontier;
    const backfill = record.historicalBackfill;
...
function connectorSynchronizationLabel(record, outcome) {
    if (outcome.kind === 'cooling_down')
        return 'Provider cooling down';
    if (outcome.kind === 'action_required')
        return 'Authentication required';
    if (outcome.kind === 'caught_up')
        return 'Caught up';
    if (outcome.kind === 'boundary_exhausted')
...
function formatList(record) {
    ...
    if (offset !== undefined)
        headerParts.push(`offset ${offset}`);
...
function isLifecycleResult(record) {
    ...
    if (status === 'blocked')
        return isPlainRecord(record.blocker);
...
function formatLifecycleResult(record) {
    const status = String(record.status);
    ...
    if (status === 'blocked') {
        ...
        if (typeof blocker?.field === 'string')
            lines.push(`Field: ${blocker.field}`);
    ...
    if (isPlainRecord(record.duplicateResolution)) {
        lines.push(`Duplicate resolution: ${String(record.duplicateResolution.action)} ${String(record.duplicateResolution.targetResourceId)}`);
    }
    return lines.join('\n');
}
function formatResolutionOutput(record) {
    const status = primitiveString(record.status);
    if (status === 'duplicate_blocked')
        return formatDuplicateBlocked(record);
    if (status === 'company_assignment_blocked')
        return formatAssignmentBlocked(record);
    if (status === 'blocked' && isPlainRecord(record.failure))
        return formatFailureBlocked(record);
...
function formatDuplicateBlocked(record) {
    ...
    for (const job of jobs) {
        if (!isPlainRecord(job))
            continue;
        lines.push(`- job=${String(job.jobId)} facts-revision=${String(job.jobFactsRevision)} company=${String(job.companyId)} company-revision=${String(job.companyRevision)} assignment-revision=${String(job.assignmentRevision)}`);
    }
    if (jobs.length === 0)
        lines.push('Conflicting Jobs: none');
    lines.push(`Allowed: ${decisions}`);
    return lines.join('\n');
}
function formatAssignmentBlocked(record) {
    const lines = [
    ...
    if (Array.isArray(record.allowedRecovery))
        lines.push(`Allowed: ${record.allowedRecovery.join(', ')}`);
    return lines.join('\n');
}
function formatFailureBlocked(record) {
    ...
    if (recovery?.action)
        lines.push(`Recovery: ${String(recovery.action)}`);
    if (Array.isArray(recovery?.guards)) {
        for (const guard of recovery.guards) {
            if (isPlainRecord(guard))
                lines.push(`Stale guard: ${formatStaleGuard(guard)}`);
        }
    }
    return lines.join('\n');
}
function formatStaleGuard(guard) {
    ...
    if (guard.kind === 'capture_revision')
        return `capture revision expected=${expected} current=${current}`;
    if (guard.kind === 'generation')
        return `generation expected=${expected} current=${current}`;
    if (guard.kind === 'company_revision')
        return `company=${String(guard.companyId)} revision expected=${expected} current=${current}`;
    if (guard.kind === 'assignment_revision')
        return `job=${String(guard.jobId)} assignment revision expected=${expected} current=${current}`;
    if (guard.kind === 'duplicate_candidate_revision')
        return `candidate=${String(guard.candidateId)} revision expected=${expected} current=${current}`;
    return `kind=${String(guard.kind)} expected=${expected} current=${current}`;
}
function formatCompletionDetail(record) {
...
function formatExactEvidenceReferences(value) {
    if (!Array.isArray(value) || value.length === 0)
        return ['Exact evidence references: none'];
    ...
    for (const reference of value.slice(0, 50)) {
        ...
        const indexes = Array.isArray(reference.evidenceIndexes)
            ...
            : 'none';
        lines.push(`- capture=${String(reference.captureId)} revision=${String(reference.captureRevision)} indexes=${indexes || 'none'}`);
    }
    if (value.length > 50)
        lines.push(`Showing first 50 references from the contract-bounded result.`);
    return lines;
}
...
function summarizeItem(value) {
    ...
    if (isConnectorRun(value)) {
        return `Connector run ${String(value.id)} - ${connectorSynchronizationLabel(value, value.outcome)}`;
    }
    ...
    const priorityScore = primitiveString(value.priorityScore) ?? primitiveString(value.currentPriorityScore);
    const priorityBand = primitiveString(value.priorityBand) ?? primitiveString(value.currentPriorityBand);
...
function isDisplayablePrimitive(value) {
