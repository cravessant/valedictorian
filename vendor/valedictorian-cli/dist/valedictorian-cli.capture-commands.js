...
import { captureHistoryInputSchema, captureListInputSchema, correctCaptureInputSchema, createCaptureInputSchema, promoteCaptureToJobInputSchema, removalInputSchema, restoreInputSchema, } from '@sparxie/sdk';
...
import { actorOptionalFlags, historyInputFlags, inputJsonFlags, listInputFlags, parseCaptureCreateInput, parseContractInput, parsePromotionInput, parseRemovalInput, parseRestoreInput, promotionOptionalFlags, removalRequiredFlags, restoreRequiredFlags, } from './valedictorian-cli.lifecycle-input.js';
    ...
    return buildRouteMap({
        ...
        routes: {
            ...
            create: makeCommand({
                ...
                flags: optionFlags(['evidence-json', 'payload-json', 'provider-record-id', 'provider-schema', 'workspace'], ['adapter-id', 'adapter-kind', 'adapter-version', 'evidence-mode', 'observed-at']),
                run: async (context, flags) => {
                    const client = await workspaceClient(context, flags);
                    writeJson(context, await client.captures.create(parseCaptureCreateInput(flags, createCaptureInputSchema)));
                },
            }),
            correct: makeCommand({
            ...
            remove: makeCommand({
                ...
                run: async (context, flags, captureId) => {
                    const client = await workspaceClient(context, flags);
                    writeJson(context, await client.captures.remove(parseRemovalInput(flags, removalInputSchema, captureId)));
                },
            }),
            restore: makeCommand({
                ...
                run: async (context, flags, captureId) => {
                    const client = await workspaceClient(context, flags);
                    writeJson(context, await client.captures.restore(parseRestoreInput(flags, restoreInputSchema, captureId)));
                },
            }),
            history: makeCommand({
