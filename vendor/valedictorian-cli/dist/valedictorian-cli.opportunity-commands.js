...
import { createOpportunityInputSchema, opportunityHistoryInputSchema, opportunityListInputSchema, promoteOpportunityToApplicationInputSchema, removalInputSchema, restoreInputSchema, updateOpportunityDispositionInputSchema, updateOpportunityEvaluationInputSchema, } from '@sparxie/sdk';
...
export function buildOpportunitiesRoute() {
    return buildRouteMap({
        ...
        routes: {
            ...
            create: inputMutation('Create an opportunity', createOpportunityInputSchema, (client, input) => client.opportunities.create(input)),
            'update-evaluation': identifiedMutation('Update opportunity evaluation', updateOpportunityEvaluationInputSchema, (client, input) => client.opportunities.updateEvaluation(input)),
            'update-disposition': identifiedMutation('Update opportunity disposition', updateOpportunityDispositionInputSchema, (client, input) => client.opportunities.updateDisposition(input)),
            remove: makeCommand({
                ...
                run: async (context, flags, opportunityId) => {
                    const client = await workspaceClient(context, flags);
                    writeJson(context, await client.opportunities.remove(parseRemovalInput(flags, removalInputSchema, opportunityId)));
                },
            }),
            restore: makeCommand({
                ...
                run: async (context, flags, opportunityId) => {
                    const client = await workspaceClient(context, flags);
                    writeJson(context, await client.opportunities.restore(parseRestoreInput(flags, restoreInputSchema, opportunityId)));
                },
            }),
            history: makeCommand({
