...
import { parseStrictJsonArray, parseStrictJsonObject, parseStrictJsonValue, readRequiredText, } from './valedictorian-cli.parser-options.js';
export function parseContractInput(flags, schema, options = {}) {
...
export function parseRemovalInput(flags, schema, id) {
...
export function parseRestoreInput(flags, schema, id) {
    return validateContract(schema, {
        id,
        actor: parseActor(flags),
        rationale: requiredFlag(flags, 'rationale'),
...
export function parsePromotionInput(flags, schema, identity) {
    ...
    if (idempotencyKey !== undefined)
        input.idempotencyKey = readRequiredText(idempotencyKey, '--idempotency-key');
    applyOverrideFlags(input, flags);
    applyDuplicateFlags(input, flags);
    return validateContract(schema, input);
}
export function parseCaptureCreateInput(flags, schema) {
...
function applyOverrideFlags(input, flags) {
...
function applyDuplicateFlags(input, flags) {
...
function validateContract(schema, input) {
...
function rejectEmbeddedIdentity(input, field) {
